# -*- coding: utf-8 -*-
# Save as: forms/khaata_meat_form.py
...
class KhaataMeatForm(ctk.CTkFrame):
    ...
    def ensure_database_schema_layers(self):
        ...
        try:
            ...
            # 1. Main expenses master infrastructure lookup table (Just in case)
            cur.execute("""
            ...
            # 2. Sub-Khaata specialized metrics ledger schema execution map for Meat (Mutton/Beef)
            cur.execute("""
                CREATE TABLE IF NOT EXISTS khaata_meat_mutton_beef (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    category VARCHAR(100) DEFAULT 'Meat (Mutton/Beef)',
    ...
    def setup_ui_components(self):
        ...
        lbl_screen_header = ctk.CTkLabel(
            editor_frame, text="Meat (Mutton/Beef) Ledger",
        ...
        ctk.CTkLabel(scroll_container, text="Item Description (e.g. Mutton, Beef, Keema):", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(4, 0))
        ...
        ctk.CTkLabel(scroll_container, text="Supplier / Butcher Name (Butchery):", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(6, 0))
        ...
        ctk.CTkLabel(qty_block, text="Quantity (KG):", text_color="#cdd6f4", font=("Segoe UI", 11)).pack(anchor="w", padx=5)
        ...
        ctk.CTkLabel(rate_block, text="Price Per KG:", text_color="#cdd6f4", font=("Segoe UI", 11)).pack(anchor="w", padx=5)
        ...
        ctk.CTkLabel(
            header_actions_row, text="Meat Transaction History",
    ...
    def refresh_booking_dropdown_list(self):
        ...
        try:
            ...
            print(f"[Dropdown Sync Warning] Dropped connections queries pipelines maps: {query_fault}")
    ...
    def load_selected_row_to_form(self, record_id):
        ...
        try:
            ...
            cur.execute("""
                ...
                FROM khaata_meat_mutton_beef WHERE id = %s
            """, (record_id,))
            res = cur.fetchone()
            conn.close()
    ...
    def execute_save_transaction_pipeline(self):
        """Handles double entry pipeline inserting into both meat sub-khaata and shared expenses rows."""
        ...
        try:
            ...
            expense_narrative_reference = f"Supplier (Butcher): {self.v_supplier.get().strip()} | Item: {self.v_item.get().strip()}"
            ...
            """, (
                ...
                advance,  # actual liquid cash drawn out from counter
            ...
            # --------------------------------------------------------------------------
            # SAVE PRIMARY ENTRIES BLOCK INTO SPECIFIC SUB-KHAATA TABLE
            # --------------------------------------------------------------------------
            cur.execute("""
                INSERT INTO khaata_meat_mutton_beef (
            ...
            messagebox.showinfo("Double Entry Log Success", "Transaction updated into Meat Sub-Khaata and synced onto Master Expense stream.")
            self.clear_form_input_fields()
            self.refresh_table_dataset()
            
            # Trigger real time metric calculations update triggers back onto root workspace monitors
    ...
    def execute_update_transaction_pipeline(self):
        ...
        try:
            ...
            cur.execute("SELECT linked_expense_id FROM khaata_meat_mutton_beef WHERE id = %s", (self.selected_id,))
            ...
            # Mutate sub-khaata local matrix schema parameters for meat
            cur.execute("""
                UPDATE khaata_meat_mutton_beef SET 
    ...
    def execute_purge_transaction_pipeline(self):
        ...
        verify_action = messagebox.askyesno("Destructive Security Request", "Are you sure you want to delete this meat transaction and remove its synced record from Master Expenses?")
        ...
        try:
            ...
            cur.execute("DELETE FROM khaata_meat_mutton_beef WHERE id = %s", (self.selected_id,))
    ...
    def execute_excel_export_matrix(self):
        ...
        try:
            cur = conn.cursor()
            cur.execute("SELECT * FROM khaata_meat_mutton_beef ORDER BY purchase_date DESC")
            dataset = cur.fetchall()
