# -*- coding: utf-8 -*-
# Save as: forms/khaata_chicken_form.py
...
class KhaataChickenPoultryForm(ctk.CTkFrame):
    ...
    def ensure_database_schema_layers(self):
        ...
        try:
            ...
            # 2. Sub-Khaata specialized metrics ledger schema execution map
            cur.execute("""
                CREATE TABLE IF NOT EXISTS khaata_chicken_poultry (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    category VARCHAR(100) DEFAULT 'Chicken & Poultry',
    ...
    def setup_ui_components(self):
        ...
        lbl_screen_header = ctk.CTkLabel(
            editor_frame, text="Poultry & Chicken Ledger",
        ...
        ctk.CTkLabel(scroll_container, text="Item Description (e.g. Broiler, Desi):", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(4, 0))
        ...
        ctk.CTkLabel(scroll_container, text="Supplier / Vendor Name Name:", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(6, 0))
        ...
        self.dropdown_bookings = ctk.CTkOptionMenu(
            ...
            fg_color="#1e1e2e", button_color="#45475a", dropdown_fg_color="#1e1e2e", text_color="#cdd6f4"
        )
        ...
        ctk.CTkLabel(
            header_actions_row, text="Poultry Transaction History",
    ...
    def load_selected_row_to_form(self, record_id):
        ...
        try:
            ...
            cur.execute("""
                ...
                FROM khaata_chicken_poultry WHERE id = %s
            """, (record_id,))
            res = cur.fetchone()
            conn.close()
    ...
    def execute_save_transaction_pipeline(self):
        """Handles double entry pipeline inserting into both chicken sub-khaata and shared expenses rows."""
        ...
        try:
            ...
            # --------------------------------------------------------------------------
            # Dynamic conditional tracking status assignments
            expense_status = "Paid"
            ...
            """, (
                ...
                advance,  # actual money liquid layout cash drawn out today
            ...
            # --------------------------------------------------------------------------
            cur.execute("""
                INSERT INTO khaata_chicken_poultry (
            ...
            messagebox.showinfo("Double Entry Log Success", "Transaction updated into Sub-Khaata and synced onto Master Expense stream.")
    ...
    def execute_update_transaction_pipeline(self):
        ...
        try:
            ...
            cur.execute("SELECT linked_expense_id FROM khaata_chicken_poultry WHERE id = %s", (self.selected_id,))
            ...
            # Mutate sub-khaata local matrix schema parameters
            cur.execute("""
                UPDATE khaata_chicken_poultry SET 
    ...
    def execute_purge_transaction_pipeline(self):
        ...
        verify_action = messagebox.askyesno("Destructive Security Request", "Are you sure you want to delete this sub-khaata transaction and remove its synced record from Master Expenses?")
        ...
        try:
            ...
            cur.execute("DELETE FROM khaata_chicken_poultry WHERE id = %s", (self.selected_id,))
    ...
    def execute_excel_export_matrix(self):
        ...
        try:
            cur = conn.cursor()
            cur.execute("SELECT * FROM khaata_chicken_poultry ORDER BY purchase_date DESC")
            dataset = cur.fetchall()
