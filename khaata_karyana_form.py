# -*- coding: utf-8 -*-
# Save as: forms/khaata_karyana_form.py
...
class KhaataKaryanaForm(ctk.CTkFrame):
    ...
    def ensure_database_schema_layers(self):
        ...
        try:
            ...
            # 2. Sub-Khaata specialized metrics ledger schema execution map for Karyana Inventory
            cur.execute("""
                CREATE TABLE IF NOT EXISTS khaata_karyana (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    category VARCHAR(100) DEFAULT 'Karyana Inventory',
    ...
    def setup_ui_components(self):
        ...
        lbl_screen_header = ctk.CTkLabel(
            editor_frame, text="Karyana Inventory Ledger",
        ...
        ctk.CTkLabel(scroll_container, text="Item Description (Rice, Ghee, Spices, Sugar, etc.):", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(4, 0))
        ...
        ctk.CTkLabel(scroll_container, text="Supplier / Karyana Store Name:", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(6, 0))
        ...
        ctk.CTkLabel(qty_block, text="Quantity (KG/Pack):", text_color="#cdd6f4", font=("Segoe UI", 11)).pack(anchor="w", padx=5)
        ...
        ctk.CTkLabel(
            header_actions_row, text="Karyana Transaction History",
    ...
    def load_selected_row_to_form(self, record_id):
        ...
        try:
            ...
            cur.execute("""
                ...
                FROM khaata_karyana WHERE id = %s
            """, (record_id,))
            res = cur.fetchone()
            conn.close()
    ...
    def execute_save_transaction_pipeline(self):
        """Handles double entry pipeline inserting into both karyana sub-khaata and shared expenses rows."""
        ...
        try:
            ...
            # --------------------------------------------------------------------------
            # SAVE PRIMARY ENTRIES BLOCK INTO SPECIFIC KARYANA SUB-KHAATA TABLE
            # --------------------------------------------------------------------------
            cur.execute("""
                INSERT INTO khaata_karyana (
            ...
            messagebox.showinfo("Double Entry Log Success", "Transaction updated into Karyana Sub-Khaata and synced onto Master Expense stream.")
    ...
    def execute_update_transaction_pipeline(self):
        ...
        try:
            ...
            cur.execute("SELECT linked_expense_id FROM khaata_karyana WHERE id = %s", (self.selected_id,))
            ...
            # Mutate sub-khaata local matrix schema parameters for Karyana
            cur.execute("""
                UPDATE khaata_karyana SET 
    ...
    def execute_purge_transaction_pipeline(self):
        ...
        try:
            ...
            cur.execute("DELETE FROM khaata_karyana WHERE id = %s", (self.selected_id,))
    ...
    def execute_excel_export_matrix(self):
        ...
        try:
            cur = conn.cursor()
            cur.execute("SELECT * FROM khaata_karyana ORDER BY purchase_date DESC")
            dataset = cur.fetchall()
