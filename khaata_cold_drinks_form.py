# -*- coding: utf-8 -*-
# Save as: forms/khaata_cold_drinks_form.py
...
class KhaataColdDrinksForm(ctk.CTkFrame):
    ...
    def ensure_database_schema_layers(self):
        ...
        try:
            ...
            # 2. Sub-Khaata specialized metrics ledger schema execution map for Cold Drinks
            cur.execute("""
                CREATE TABLE IF NOT EXISTS khaata_cold_drinks (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    category VARCHAR(100) DEFAULT 'Cold Drinks & Beverages',
                    item_name VARCHAR(100) NOT NULL,
                    quantity INT NOT NULL,
                    unit VARCHAR(20) DEFAULT 'Crates/Boxes',
    ...
    def initialize_form_variables(self):
        ...
        self.v_unit = tk.StringVar(value="Crates")
    ...
    def setup_ui_components(self):
        ...
        lbl_screen_header = ctk.CTkLabel(
            editor_frame, text="Cold Drinks Ledger",
        ...
        ctk.CTkLabel(scroll_container, text="Drink Name / Brand (e.g., Coke, Sprite, Dew):", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(4, 0))
        ...
        ctk.CTkLabel(scroll_container, text="Supplier / Agency Name:", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(6, 0))
        ...
        # Inputs Dual Matrix: Quantities, Units and Rates
        ...
        qty_block.grid(row=0, column=0, padx=(0, 2), sticky="nsew")
        ctk.CTkLabel(qty_block, text="Quantity:", text_color="#cdd6f4", font=("Segoe UI", 11)).pack(anchor="w", padx=5)
        ctk.CTkEntry(qty_block, textvariable=self.v_qty, fg_color="#1e1e2e", border_color="#45475a").pack(fill="x", pady=2)
        
        unit_block = ctk.CTkFrame(metrics_row, fg_color="transparent")
        unit_block.grid(row=0, column=1, padx=2, sticky="nsew")
        ctk.CTkLabel(unit_block, text="Unit Typology:", text_color="#cdd6f4", font=("Segoe UI", 11)).pack(anchor="w", padx=5)
        ctk.CTkOptionMenu(unit_block, variable=self.v_unit, values=["Crates", "Boxes", "Bottles", "Loose Packs"], fg_color="#1e1e2e", button_color="#45475a").pack(fill="x", pady=2)
        
        rate_block = ctk.CTkFrame(metrics_row, fg_color="transparent")
        rate_block.grid(row=0, column=2, padx=(2, 0), sticky="nsew")
        ...
        ctk.CTkLabel(scroll_container, text="Cash Paid Out (Liquid Expense Cash):", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(6, 0))
        ...
        ctk.CTkLabel(scroll_container, text="Link to Event Booking Slot:", text_color="#cdd6f4", font=("Segoe UI", 12)).pack(anchor="w", padx=5, pady=(6, 0))
        ...
        ctk.CTkLabel(calculated_hud_panel, text="Outstanding Balance (Udhaar):", text_color="#a6adc8", font=("Segoe UI", 11, "bold")).grid(row=0, column=1, padx=12, pady=(8, 2), sticky="e")
        ...
        ctk.CTkLabel(
            header_actions_row, text="Cold Drinks Transaction History",
    ...
    def refresh_table_dataset(self):
        """Flushes and loads current rows sets blocks from database schemas inside views elements."""
        ...
        col_mappings = [("Beverage Details", 0.35), ("Purchase Date", 0.20), ("Total Outlay", 0.25), ("Balance Due", 0.20)]
        ...
        try:
            ...
            for row in rows:
                db_id, item, qty, unit, price, total, advance, balance, supplier, p_date, b_id, exp_id = row
    ...
    def load_selected_row_to_form(self, record_id):
        ...
        try:
            cur = conn.cursor()
            cur.execute("""
                SELECT item_name, quantity, unit, price_per_unit, advance_paid, supplier_name, purchase_date, booking_id 
                FROM khaata_cold_drinks WHERE id = %s
            """, (record_id,))
            res = cur.fetchone()
            conn.close()
            
            if res:
                item, qty, unit, price, advance, supplier, p_date, b_id = res
                self.v_item.set(item)
                self.v_qty.set(str(qty))
                self.v_unit.set(unit)
    ...
    def execute_save_transaction_pipeline(self):
        """Handles double entry pipeline inserting into both cold drinks sub-khaata and shared expenses rows."""
        if not self.v_item.get().strip() or not self.v_supplier.get().strip():
            messagebox.showwarning("Validation Error", "Beverage description and Supplier credentials cannot be blank.")
            return
        ...
        try:
            ...
            expense_narrative_reference = f"Supplier: {self.v_supplier.get().strip()} | Items: {self.v_item.get().strip()} ({int(qty)} {self.v_unit.get()})"
            ...
            # --------------------------------------------------------------------------
            # SAVE PRIMARY ENTRIES BLOCK INTO SPECIFIC COLD DRINKS SUB-KHAATA TABLE
            # --------------------------------------------------------------------------
            cur.execute("""
                INSERT INTO khaata_cold_drinks (
                    item_name, quantity, unit, price_per_unit, total_amount, advance_paid, 
            ...
            messagebox.showinfo("Double Entry Log Success", "Transaction updated into Cold Drinks Ledger and synced onto Master Expense stream.")
    ...
    def execute_update_transaction_pipeline(self):
        ...
        try:
            ...
            cur.execute("SELECT linked_expense_id FROM khaata_cold_drinks WHERE id = %s", (self.selected_id,))
            ...
            # Mutate sub-khaata local matrix schema parameters for Cold Drinks
            cur.execute("""
                UPDATE khaata_cold_drinks SET 
                    item_name = %s, quantity = %s, unit = %s, price_per_unit = %s, total_amount = %s, 
                    advance_paid = %s, remaining_balance = %s, supplier_name = %s, 
                    purchase_date = %s, booking_id = %s
                WHERE id = %s
            """, (
                self.v_item.get().strip(), int(qty), self.v_unit.get(), rate, total_outlay, advance, remaining_due,
    ...
    def execute_purge_transaction_pipeline(self):
        ...
        verify_action = messagebox.askyesno("Destructive Security Request", "Are you sure you want to delete this beverage record and remove its synced entry from Master Expenses?")
        ...
        try:
            ...
            cur.execute("DELETE FROM khaata_cold_drinks WHERE id = %s", (self.selected_id,))
    ...
    def execute_excel_export_matrix(self):
        ...
        try:
            cur = conn.cursor()
            cur.execute("SELECT * FROM khaata_cold_drinks ORDER BY purchase_date DESC")
            dataset = cur.fetchall()
